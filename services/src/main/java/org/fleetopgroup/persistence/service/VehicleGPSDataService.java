	...
	public ValueObject getFleetexRunKM(ValueObject valueObject) throws Exception {
	 ...
	 public ValueObject getEicherRunKM(ValueObject valueObject) throws Exception {
			...
			try {
				...
				LocalDateTime fromDate = LocalDateTime.parse(valueObject.getString("fromDate"), inputFormatter);
			    LocalDateTime toDate = LocalDateTime.parse(valueObject.getString("toDate"), inputFormatter);
	    		...
	    		noOfDays          =  (int) gpsConfig.getOrDefault("noOfDays","");
	    		if(url.length() > 0) {
	    			
	    	        for (int i = 0; i <= daysDifference; i += noOfDays) {
	    	        	gpsUsageKM				= 0.0;
	    	            LocalDateTime chunkFromDate = fromDate.plusDays(i);
	    	            LocalDateTime chunkToDate = null;
	    	            
	    	            if(daysDifference == 0 || i==(daysDifference-1)) {
	    	            	 chunkToDate = toDate;
	    	            }else if(daysDifference >0) {
		   	            	 chunkToDate = fromDate.plusDays(Math.min(i + 2, daysDifference));
		   	            }
	    	            
	    	            json.put("startDateTime", Long.parseLong(chunkFromDate.format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"))));
	    	            json.put("endDateTime", Long.parseLong(chunkToDate.format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"))));
		    		    				...
		    		    				if(jsonObject.has("positiondata")) {
			    		    				...
			    		    				for(Object jsonObj : positionDataArray) {
			    		    					...
			    		    					try {
				    		    					...
				    		    					if(gpsDto.getOdometer() != null && gpsDto.getOdometer() > 0 && JSON.get("ignitionStatus") != null && JSON.getInt("ignitionStatus") > 0) {
				    		    						gpsDtoList.add(gpsDto);
				    		    						JSONObjectlist.put(JSON);
				    		    					}
	    		...
	    		valueObject.put("gpsUsageKM", TotalgpsUsageKM);
