		...
		try {
			xSync.execute(valueInObject.getLong("driverId",0), () -> {
				...
				try {
					...
					Timestamp txnTime = DateTimeUtility.getTimeStamp(DateTimeUtility.getCurrentDateStr(DateTimeUtility.YYYY_MM_DD_HH_MM_SS),
...
public void transferDriverBalanceToNextTrip(ValueObject valueInObject,DriverTripSheetBalance preDriverBalance,DriverLedger preDriverLedger) {
	...
	@Override
	public void updateDriverLedgerWhenDriverUpdated(long tripSheetId,long tripNumber,long driverId,long oldDriverId) {
		
		try {
			CustomUserDetails userDetails = Utility.getObject(null);
			updateDriverLedgerWhenDriverUpdated(tripSheetId,tripNumber, oldDriverId,userDetails.getCompany_id(),userDetails.getId(),true);
			updateDriverLedgerWhenDriverUpdated(tripSheetId,tripNumber,driverId,userDetails.getCompany_id(),userDetails.getId(),false);
	...
	public void updateDriverLedgerWhenDriverUpdated(long tripSheetId,long tripNumber,long driverId,int companyId,long userId,boolean forOldDriver) throws Exception {
		...
		if(advanceList != null && !advanceList.isEmpty()){
			...
			if (forOldDriver) {
				...
				HashMap<String, Object>  config	= companyConfigurationService.getCompanyConfiguration(companyId, PropertyFileConstants.TRIPSHEET_CONFIGURATION_CONFIG);
				if((boolean) config.getOrDefault("transferDriverBalanceToNextTrip", false))
