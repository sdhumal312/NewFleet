					...
					if (validate != null && !validate.isEmpty()) {
						...
						if((boolean) gpsConfiguration.get(VehicleGPSConfiguratinConstant.ALLOW_GPS_INTEGRATION) && CheckVehicleStatus.getGpsVendorId() != null && CheckVehicleStatus.getGpsVendorId() > 0) {
								try {
									...
									if(preFuel != null) {
										try {
											gpsUsageKM	=	vehicleGPSDetailsService.getGPSRunKMBetweenTwoDates(fuel.getVid(),preFuel.getFuelDateTimeStr()+".0", fuelDate+".0" , companyId);
										} catch (Exception e) {
		...
		try {    		 
			...
			if((boolean) gpsConfiguration.get(VehicleGPSConfiguratinConstant.ALLOW_GPS_INTEGRATION) && CheckVehicleStatus.getGpsVendorId() != null && CheckVehicleStatus.getGpsVendorId() > 0) {
				try {
					...
					if(preFuel != null) {
						try {
							gpsUsageKM	=	vehicleGPSDetailsService.getGPSRunKMBetweenTwoDates(fuel.getVid(),preFuel.getFuelDateTimeStr()+".0", fuelDate+".0" , userDetails.getCompany_id());
						} catch (Exception e) {
							System.err.println("Exception while getting gps run km fuel "+e);
						}
				...
				if (fuel.getPaymentTypeId() == PaymentTypeConstant.PAYMENT_TYPE_CASH || fuel.getPaymentTypeId() != PaymentTypeConstant.PAYMENT_TYPE_CREDIT) {
					...
					try {
						...
						if(fuel.getFuel_TripsheetID() != null && fuel.getFuel_TripsheetID() > 0) {
							...
							TripSheetExpense expense=tripSheetExpenseRepository.getTripSheetExpByFuelId(fuel.getFuel_id());
							if(oldFuel.getPaymentTypeId() != fuel.getPaymentTypeId() || ( oldPaidByBranchId >0 && fuel.getPaidByBranchId() <=0 && expense==null)) {
								System.err.println("inside add expense OR  condition ");
								tripSheetService.addTripSheetExpense(TripExpense);
							...
							if(!fuel.getFuel_amount().equals(previousFuelAmount)) {
								double diffAmount = previousFuelAmount - fuel.getFuel_amount();
								...
								if(TripExpense != null) {
									...
									if(trip.getTripStutesId() == TripSheetStatus.TRIP_STATUS_CLOSED && (boolean)tripConfiguration.get("addInDriverbalanceAfterTripClose")) {
										driverBalance = trip.getDriverBalance();
										if(driverBalance != null && diffAmount > 0) 
											driverBalance = driverBalance + diffAmount;
										else if(driverBalance != null && diffAmount < 0)
											driverBalance =  driverBalance + diffAmount;
					...
					if(fuel.getFuel_TripsheetID() != null && fuel.getFuel_TripsheetID() > 0 && oldFuel.getPaymentTypeId() != fuel.getPaymentTypeId()) {
						...
						if(trip.getTripStutesId() == TripSheetStatus.TRIP_STATUS_CLOSED && (boolean)tripConfiguration.get("addInDriverbalanceAfterTripClose")) {
								driverBalance = trip.getDriverBalance();
								if(driverBalance != null) 
									driverBalance = driverBalance + previousFuelAmount;
								else
									driverBalance =  previousFuelAmount;
	...
	public ValueObject updateFuelGPSUsage(ValueObject object) throws Exception {
		...
		try {
			...
			if(currentFuel != null && previousFuel != null) {
				gpsUSageKM	=	vehicleGPSDetailsService.getGPSRunKMBetweenTwoDates(currentFuel.getVid(), format.format(previousFuel.getFuelDateTime())+".0", format.format(currentFuel.getFuelDateTime())+".0", currentFuel.getCompanyId());			
...
@Override
	...
	try {
		Query query = entityManager.createQuery(
				...
				+ " VH.vehicle_registration, VH.ledgerName, F.created, F.paymentTypeId,F.vid, "
				...
				+ " AND F.companyId = "+companyId+" AND F.markForDelete = 0 AND F.fuel_amount > 0 AND F.paymentTypeId = "+PaymentTypeConstant.PAYMENT_TYPE_CREDIT+" ");
		...
		if (results != null && !results.isEmpty()) {
			...
			for (Object[] vehicle : results) {
				if (vehicle != null) {
					...
					select.setCreated((Date) vehicle[7]);
					...
					if(select.getCreated() != null ) {
					 ...
					 select.setRemark("Fuel Entry On Vehicle "+select.getVehicle_registration()+" Date: "+dateFormat.format((java.util.Date)vehicle[12])+" Fuel No : "+select.getTripSheetNumberStr()+" From: "+select.getVendorName()+" Fuel Reference : "+vehicle[13]);
		...
		public ValueObject saveTripsheetFuelDetailsFromClosTripApi(ArrayList<ValueObject> fulearray, ValueObject object, Vehicle vehicle)
			...
			if (fulearray != null) {
				...
				trip = tripSheetService.getTripByTripsheetID(object.getLong("tripsheetId"));

				
				for(ValueObject fuelvalueObj: fulearray) {
			        try {
			        	usage += fuelvalueObj.getInt("fuel_meter",0);
			        	...
			        	if(vendor != null)
			        		...
			        		if(fuel != null) {
				        		...
				        		fuelvalueObj.put("fuel_meter", fuelvalueObj.getInt("fuel_meter_old",0)+fuelvalueObj.getInt("fuel_meter",0));
			        		}
		...
		public Fuel getFuelMeterByDateTime(Integer vid, Integer companyId) throws Exception {
			// TODO Auto-generated method stub
			try {
		        Query query = entityManager.createQuery("FROM Fuel AS F WHERE F.vid=?1 AND F.companyId = ?2 AND F.markForDelete = 0 " +
		                "ORDER BY F.fuelDateTime DESC");
