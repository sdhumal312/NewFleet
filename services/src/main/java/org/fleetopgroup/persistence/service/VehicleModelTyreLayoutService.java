...
public class VehicleModelTyreLayoutService implements IVehicleModelTyreLayoutService {
	...
	public ValueObject saveVehicleModelTyreLayout2(ValueObject valueObject) throws Exception {
		VehicleModelTyreLayout						vehicleModelTyreLayout						= null;
		List<Vehicle>								vehicleList									= null;
		VehicleTyreLayoutPosition       			vehicleTyreLayoutPosition					= null;
		List<VehicleTyreLayoutPosition>       		vehicleTyreLayoutPositionList				= null;
		List<VehicleModelTyreLayoutPosition> 		vehicleModelTyreLayoutPositions				= null;
		...
		try {
			...
			List<VehicleModelDto>    vehicleModelList = vehicleModelService.getVehicleModelListByCompanyId(userDetails.getCompany_id());
			
			if(vehicleModelList != null && !vehicleModelList.isEmpty()) {
				for (VehicleModelDto vehicleModelDto : vehicleModelList) {
					
					vehicleModelTyreLayout			= vModalTyreLayoutDao.getvehicleTyreModelLayoutByModelId(vehicleModelDto.getVehicleModelId() ,userDetails.getCompany_id());
					if(vehicleModelTyreLayout != null ) {
						
						vehicleModelTyreLayoutPositions	= vModalTyreLayoutPositionDao.getBYvehicleModelTyreLayoutId(vehicleModelTyreLayout.getVehicleModelTyreLayoutId());
						vehicleList 					= vehicleDao.getvehicleByModelId(vehicleModelDto.getVehicleModelId(),userDetails.getCompany_id());
						
						if(vehicleList != null && !vehicleList.isEmpty()) {
							for(Vehicle vehicle : vehicleList) {
								List<VehicleTyreLayoutPosition> validate	 = vehicleTyreLayoutPositionRepository.Get_Vehicle_Type_Details(
																					vehicle.getVid(),vehicle.getCompany_Id());
								if(validate == null || validate.isEmpty()) {
									for(VehicleModelTyreLayoutPosition  layoutPosition : vehicleModelTyreLayoutPositions) {
										vehicleTyreLayoutPosition = vModelTyreLayoutBL.prepareMultipleVehicleTyreLayoutPosition(layoutPosition.getPosition(),
												layoutPosition.getTyreModelId(),vehicle.getVid() , vehicle.getCompany_Id());
										vehicleTyreLayoutPositionList.add(vehicleTyreLayoutPosition);
									}
								}
							}
