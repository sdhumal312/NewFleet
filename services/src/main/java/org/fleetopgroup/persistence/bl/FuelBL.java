...
public class FuelBL {
	...
	public FuelDto getFuelId(FuelDto fuelBean) {
		...
		try {
			if(fuelBean != null)
				...
				fuel.setPaidByBranchStr(fuelBean.getPaidByBranchStr());
				fuel.setPaidByBranchId(fuelBean.getPaidByBranchId());
				fuel.setModuleIdentifire(fuelBean.getModuleIdentifire());
	...
	public Integer prepareTotalUsage_Tripsheet_fuel_details_Changed(List<FuelDto> showAmount) {
		Integer totalFuelusage = showAmount != null
		        ? showAmount.stream()
		                    .map(FuelDto::getFuel_usage)
	 ...
	 * @return
	 */
	public Double prepareTotalUsage_Tripsheet_fuel_kmpl(List<FuelDto> showAmount) {
		int count = 0;
		Double totalFuelusage = 0.0;
		if (showAmount != null && !showAmount.isEmpty()) {
			for (FuelDto fuelBean : showAmount) {
				if (fuelBean.getFuel_kml() != null && fuelBean.getFuel_kml() > 0) {
					totalFuelusage += fuelBean.getFuel_kml();
					count++;
				}
			}
		}
		return totalFuelusage/count;
	}
	...
	public Double prepareTotalAmount_Tripsheet_Created_fuel_details_New(List<FuelDto> showAmount) {
		Double totalFuelamount = 0.0;
		if (showAmount != null && !showAmount.isEmpty()) {
			for (FuelDto fuelBean : showAmount) {
				if ((fuelBean.getFuel_amount() != null && fuelBean.getPaymentTypeId()!=PaymentTypeConstant.PAYMENT_TYPE_CASH)
						|| (fuelBean.getFuel_amount() != null && fuelBean.getPaidByBranchId() != null && fuelBean.getPaidByBranchId() > 0)) {
					totalFuelamount += fuelBean.getFuel_amount();
				}
			}
