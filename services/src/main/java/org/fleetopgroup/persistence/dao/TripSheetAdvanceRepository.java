...
public interface TripSheetAdvanceRepository  extends JpaRepository<TripSheetAdvance, Long>{
	...
	@Query("from TripSheetAdvance where tripAdvanceID >= ?1 AND tripAdvanceID <= ?2 AND markForDelete = 0 AND paymentTypeId = 11")
	public List<TripSheetAdvance> findAllTripSheetAdvanceByIds(Long lowerLimit, Long higherLimit) throws Exception;
