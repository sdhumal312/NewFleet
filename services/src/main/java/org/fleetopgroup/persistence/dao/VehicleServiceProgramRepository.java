...
public interface VehicleServiceProgramRepository extends JpaRepository<VehicleServiceProgram, Long> {
	...
	@Query("select v.vehicleServiceProgramId FROM VehicleServiceProgram as v Where  (v.companyId = ?1 OR v.isVendorProgram = 1) AND v.markForDelete = 0 ORDER BY v.vehicleServiceProgramId DESC ")
