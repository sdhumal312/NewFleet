	...
	public ModelAndView TSPrint(@RequestParam("ID") final Long id, final HttpServletRequest request) {
		...
		try {
			...
			if((boolean) configuration.get("reverseDriverBalance")) {
			 ...
			 if((boolean) configuration.get("addInDriverbalanceAfterTripClose") && trip.getTripStutesId() >= TripSheetStatus.TRIP_STATUS_CLOSED) {
					if(trip.getDriverBalance() == null)
						trip.setDriverBalance(0.0);
						
						model.put("driverBalance",tofixedTwo.format(trip.getDriverBalance()));
			}
				...
				fuelAmount= fuelBL.prepareTotalAmount_Tripsheet_Created_fuel_details(ShowAmount);
				model.put("fTDiesel", fuelBL.prepareTotal_Tripsheet_fuel_details(ShowAmount));
				if((boolean) configuration.get("diffFuleUsageKmlogic")){
					model.put("fTUsage", fuelBL.prepareTotalUsage_Tripsheet_fuel_details(ShowAmount));
				}else {
					model.put("fTUsage", fuelBL.prepareTotalUsage_Tripsheet_fuel_details_Changed(ShowAmount));
				}
