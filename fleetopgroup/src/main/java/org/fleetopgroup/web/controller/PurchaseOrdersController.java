...
public class PurchaseOrdersController {
	...
	public ModelAndView searchPurchaseOrders(@RequestParam("Search") final String Purchaseorder_id,
		...
		try {
			...
			HashMap<String, Object>	configuration =	companyConfigurationService.getCompanyConfiguration(userDetails.getCompany_id(), PropertyFileConstants.PURCHASE_ORDER_CONFIG);
		...
		try {
			...
			if (bankPaymentDto.isAllowBankPaymentDetails()) {
				if(PurchaseOrd.getPurchaseorder_termsId() == PaymentTypeConstant.PAYMENT_TYPE_CASH || PaymentTypeConstant.getPaymentTypeIdList().contains(PurchaseOrd.getPurchaseorder_termsId()) ) {
						...
						Vendor vendor	=  vendorService.getVendor(PurchaseOrd.getPurchaseorder_vendor_id());
						bankPaymentValueObject.put("remark", "Purchase Order Created PO-"+PurchaseOrd.getPurchaseorder_Number()+" Vendor : "+vendor.getVendorName()+" Payment Type : "+PaymentTypeConstant.getPaymentTypeName(PurchaseOrd.getPurchaseorder_termsId()));
	...
	public ModelAndView savePurchaseOrderPart(@ModelAttribute("command") PurchaseOrdersToParts purchaseOrdersTasksToParts, 
		...
		try {	
			...
			for(int i =0 ;i<partid.length;i++) {
				...
				purchaseTaskToParts= PurchaseOrdersService.preparePurchaseOrdersTaskToPart(partid[i],quantity[i],parteachcost[i],discount[i],tax[i]);
	...
	public ModelAndView PurchaseOrdersUpdate_Complete(@RequestParam("ID") final Long Purchaseorder_id,
		...
		try {
			...
			if (InvenBL.isFilled_GET_Permission(Purchase.getPurchaseorder_shiplocation_id(), PartLocPermission)) {
				...
				//bankPaymentValueObject.put("paidDate", Purchase.getPurchaseorder_invoice_date_On());
