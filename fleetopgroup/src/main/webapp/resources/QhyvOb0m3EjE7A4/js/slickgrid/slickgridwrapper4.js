...
function applyGrid(jsonInObject){
	...
	if(jsonOptions.PersistGridToAppend == false){
		slickGridInstanceGlobalObj[jsonOptions.InnerSlickId] = setSlickTable(jsonOptions); 
	}else{
		if(typeof slickGridInstanceGlobalObj[jsonOptions.InnerSlickId] == "undefined"){
			slickGridInstanceGlobalObj[jsonOptions.InnerSlickId] = setSlickTable(jsonOptions); 
		}else{
			appendDatatoSlickTable(jsonOptions);
		}
	}

}
function appendDatatoSlickTable(jsonOptions){
	var gridObj = getSlickGridInstance(jsonOptions);
...
function setSlickTable(jsonOptions){

	slickStructureTable(jsonOptions);

	var columns = []; // columns array for SlickGrid Object
	var columnPicker = []; // columns array for SlickGrid Object

	checkBoxColumn(jsonOptions,columns);
	removeButtonColumn(jsonOptions,columns);
	partialButtonColumn(jsonOptions,columns);
	serialNumberColumn(jsonOptions,columns,columnPicker);

	var buttonsObjCol = headerButton(jsonOptions);
	...
	registerSlickGridPlugins(columnPicker,jsonOptions,grid,dataView,jsonOptions.InnerSlickId);
...
function getSlickGridInstance(jsonInOptions){
	return slickGridInstanceGlobalObj[jsonInOptions.InnerSlickId];
}
function slickStructureTable(jsonOptions){
...
function checkBoxColumn(jsonOptions,columns){
...
function headerButton(jsonOptions){
	var buttonsObj = [];
...
function partialButtonColumn(jsonOptions,columns){
	var language = jsonOptions.Language;
...
function removeButtonColumn(jsonOptions,columns){
	var language = jsonOptions.Language;
...
function serialNumberColumn(jsonOptions,columns,columnPicker){
	buttonsObj = headerButton(jsonOptions);
...
function registerSlickGridPlugins(columnPicker,jsonOptions,grid,dataView,innerSlickID){
	...
	grid.onPageChanged.subscribe(function() {
		updateColumnTotal(columnPicker,dataView,totals,innerSlickID);
	});
...
function updateColumnTotal(columnPicker,dataView,totals,innerSlickID){
...
function getValueForSelectedData(jsonOptions, selectionMsg) {
	var griDobj = getSlickGridInstance(jsonOptions);
	var selectedData = [],
	selectedIndexes;
...
function updateRowColor(jsonOptions,dtoName,equateVal,cssClass){
	var slickgrid = getSlickGridInstance(jsonOptions);
...
function checkToAddRowInTable(jsonInOptions,columnData,checkDtoName){
	getSlickGridInstance(jsonInOptions);
...
function setAggregateFunction(grid,column){
