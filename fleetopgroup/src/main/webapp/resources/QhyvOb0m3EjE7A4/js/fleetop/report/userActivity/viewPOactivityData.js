		...
		$.ajax({
			...
			success : function(data){
				if(data.tableData != undefined && data.tableData.length > 0){
						...
						$('#ResultContent').show();
						if(activityType == 1){
							$("#reportHeader").html("Purchase Order Created List");
						}else if(activityType == 2){
							$("#reportHeader").html("Purchase Order Updated List");
						}else if(activityType == 3){
							$("#reportHeader").html("Purchase Order Deleted List");
						}
			...
			if(tableConfig != undefined) {
				...
				for (var i = 0; i < columnKeys.length; i++) {
					...
					if (bObj.show != undefined && bObj.show == true) {
						...
						if(userId >0 && bObj.tableDtoName == 'userName'){
							bObj.show = false;	
						}
						if(activityType == 3 && bObj.tableDtoName =='lastupdated'){
