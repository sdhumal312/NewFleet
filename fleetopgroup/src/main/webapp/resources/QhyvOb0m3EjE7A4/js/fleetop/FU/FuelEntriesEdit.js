...
function   setFuelEditData(data){
	...
	if(data.fuel.fuel_TripsheetID != null && data.fuel.fuel_TripsheetID > 0){
		$('#tripsheetNumberSelect').html('<option value= '+data.fuel.fuel_TripsheetID+' selected>TS-'+data.fuel.fuel_TripsheetNumber+'</option>');
		$('#tripsheetNumberSelect').val(data.fuel.fuel_TripsheetID);
	}
	...
	if(data.fuel.paymentTypeId == 1){
		if(data.fuel.paidByBranchId >0){
			$('#selectBranch').select2('data', {
				id : data.fuel.paidByBranchId,
				text : data.fuel.paidByBranchStr
			});
	...
	if(data.fuel.paymentTypeId > 1){
		$('#paidByOption').hide();
		selectElement = $("#renPT_option");
		
		selectElement.find("option").each(function() {
	        var option = $(this);
	        if (Number(option.val()) === data.fuel.paymentTypeId){
	            option.prop("selected", true);
	            
	            if(Number(option.val()) > 2){
	            	setBankPaymentDetails(option);
	            	getPaymentDetails(data.fuel.fuel_id,data.fuel.moduleIdentifire,false)
	            }
	            return false;
	        } else {
