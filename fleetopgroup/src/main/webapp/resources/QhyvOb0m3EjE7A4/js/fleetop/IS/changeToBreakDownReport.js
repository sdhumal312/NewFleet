$(document).ready(function(){
	$('#vehicleModelList').select2({
		placeholder : " All "
		
	});
	$('#partCategory').select2({
		placeholder : "ALL"
	})
	$('#status').select2({
		placeholder : "ALL"
...
$(document).ready(
		function($) {
			$('button[type=submit]').click(function(e) {
				...
				jsonObject["vehicleModel"] 					= $('#vehicleModelList').val();
...
function renderReportData(resultData,dateRangeVal) {
	...
	if(resultData.driverKey != "ALL"){
		$('#driverKey').html(resultData.driverKey);
		$('#driverKeyId').show()
	}
	if(resultData.vehicleModelKey != "ALL"){
		$('#modelKey').html(resultData.vehicleModelKey);
		$('#modelKeyId').show()
	}
		...
		$("#reportHeader").html("Issue Type Changed To BreakDown Report .");
		...
		if(resultData.tableConfig != undefined) {
			...
			for (var i = 0; i < columnKeys.length; i++) {
				...
				if(resultData.driverId > 0 && bObj.tableDtoName == "issues_DRIVER_NAME" ){
					bObj.show = false;
				}
