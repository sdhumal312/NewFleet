...
function createWorkOrder(){
	...
	for(var i =0 ; i< jobType.length; i++){
		...
		if(Number(service_ROT[i]) ==0 && Number(service_ROT_number[i])==0){
			workOrderDetails.jobSubType				= serviceSubJobs[i];
		}else{
			 workOrderDetails.jobSubType		    = "";
		}
...
function visibility(a, b,z) {
	$("#newJobSubId"+z).val("0");
	$('#programDetails'+z).empty();
    ...
    if (data.serviceReminderDtoList !== undefined) {
	    ...
	    var jobSubIds = [];
		 ...
		 for (var i = 0; i < list.length; i++) {
		    ...
		    var p1 = $("<p>").text("Service Program- " + ServiceList.serviceProgram + " ").append(ServiceList.diffrent_time_days);
		    p1.append(anchor);
		    div.append(p1);
