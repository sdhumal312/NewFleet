function addTollExpensesDetails(tripSheetId, vid, vehicle_registration, opentripDate, closetripDate){
		var jsonObject			= new Object();
		
		var VehicleNumber = vehicle_registration;
		
		jsonObject["StartTransactionDate"] 	  			= opentripDate;
		jsonObject["EndTransactionDate"] 	  			= closetripDate;
		jsonObject["VehicleNumber"] 					= VehicleNumber.replace(/-/g,'');
		jsonObject["tripSheetId"] 						=  tripSheetId;
		jsonObject["vid"] 								=  vid;
		jsonObject["vehicle_registration"] 				=  vehicle_registration;
		
		
		
		showLayer();
		$.ajax({
	             url: "getVehicleTollExpenseDetails",
	             type: "POST",
	             dataType: 'json',
	             data: jsonObject,
	             success: function (data) {
	            	 ...
	            	 if(data.tollExpensesAlreadyAdded){
	            		 if(data.flavor == 1){
	            			 window.location.replace("showTripSheet.in?tripSheetID="+tripSheetId+"&tollExpensesAlreadyAdded="+true+"");
	            		 }else if(data.flavor == 3){
	            			 window.location.replace("showTripDaily.in?ID="+tripSheetId+"&tollExpensesAlreadyAdded="+true+"");
	            		 }
	            	 }else if(data.tollExpensesAdded){
	            		 if(data.flavor == 1){
	            			 window.location.replace("showTripSheet.in?tripSheetID="+tripSheetId+"&tollExpensesAdded="+true+"");
	            		 }else if(data.flavor == 3){
	            			 window.location.replace("showTripDaily.in?ID="+tripSheetId+"&tollExpensesAdded="+true+"");
	            		 }
