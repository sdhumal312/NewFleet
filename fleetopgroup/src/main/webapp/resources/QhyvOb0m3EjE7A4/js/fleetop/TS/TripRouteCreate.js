$(document).ready(function() {
    var e = 25,
        ...
        a = 1;
    $(n).click(function(n) {
        n.preventDefault(), e > a && (a++, $(t).append('<div><div class="row1"><div class="col-md-3"><select class="form-text select2" name="expenseName" id="Expense' + a + '"  required="required"></select></div><div class="col-md-2"><input type="number"  name="maxAmount" onblur="validateMaxAmount('+ a +');" class="form-text" id="maxAmountOnCreate' + a + '"></div><div class="col-md-2"><input type="number" class="form-text" onkeyup="return validateExpenseRange('+a+');" name="Amount" id ="Amount' + a + '" min="0" required="required" placeholder="Amount"> </div><div class="col-md-3"><input type="text" class="form-text" name="expenseRefence" placeholder="Reference" value="R00"></div></div><a href="#" class="remove_field"><font color="FF00000"><i class="fa fa-trash"></i> Remove</a></font></div>'), $.getJSON("getTripExpenseList.in", function(e) {
...
function setExpenseRangeList(data){
	...
	if(tripRouteExpenseRangeList != undefined || tripRouteExpenseRangeList != null){
		for(var index = 0 ; index < tripRouteExpenseRangeList.length; index++){
			...
			columnArray.push("<td class='fit ar'><input type='text' maxlength='6' class='form-text' name='expenseRangeAmount' id='expenseMaxLimtId"+tripRouteExpenseRangeList[index].tripExpenseId+"' onkeypress='return isNumberKeyWithDecimal(event,this.id);' value="+ tripRouteExpenseRangeList[index].expenseMaxLimt +"> </td>");
...
function validateMaxAmount(a){
	var	expenseAmount 			= Number($("#Amount"+a).val());
	var maxAmountOnCreate 		= Number($("#maxAmountOnCreate"+a).val());
	var	maxlimitConfig			= $("#maxlimitConfig").val();
	var	expenseOutOfRange		= $("#expenseOutOfRange").val();
	
	if((expenseAmount > maxAmountOnCreate) && (maxlimitConfig == true ||  maxlimitConfig == 'true') && (expenseOutOfRange == false || expenseOutOfRange == 'false')){
		$("#Amount"+a).val(maxAmountOnCreate);
	}
}
