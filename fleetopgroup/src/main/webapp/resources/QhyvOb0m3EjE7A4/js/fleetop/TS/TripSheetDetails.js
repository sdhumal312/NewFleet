		...
		var  incomeTable = '<tr><td><table class="table table-bordered table-striped"><thead>'
				+'<tr class="breadcrumb">'
				   ...
				   if(configuration.showLsDestination){
					   incomeTable +='<th class="fit ar">LsDestinationBranch</th>'
                       +'<th class="fit ar">LS CreatedDate</th>'
				   }
				  ...
				  if(!configuration.showToPayPaidTotalOfLS){
					  ...
					  incomeTable +='<th class="fit ar" style="width:10%" >ToPay</th>'
				           			+'<th class="fit ar" style="width:5%" >Paid</th>';
				  }
				   
				  incomeTable +='<th class="fit ar" style ="width:13%" >Amount</th>'
				               ...
				               +'<td class="fit">'+TripSheetIncome[i].incomeName+'</td>'
				                ...
				                if(configuration.showWithOrWithoutBill){
								  ...
								  incomeTable +='<th class="fit ar">'+TripSheetIncome[i].lsDestinationBranch+'</th>'
                                    +'<td class="fit">'+TripSheetIncome[i].tripDateTime+'</td>';
								}
				 			   
				 			   +'<td class="fit">'+TripSheetIncome[i].incomeRefence+'</td>';
	...
	if(data.fuel != undefined && data.fuel.length > 0){
		...
		var kmSign = '';
			...
			if(data.fTUsage > 0){
				...
				if($("#showFuelKmplOnFullFuelEntry").val() == true || $("#showFuelKmplOnFullFuelEntry").val() == 'true')
					fuelKmpl= data.fuelKmplForFull;
			}
			if(data.fTUsage > 0){
		...
		if($("#showDiffAvgCost").val() == true || $("#showDiffAvgCost").val() == 'true'){
			...
			incomeTable += '<tr><td class="key" colspan="5"><b>Total :</b></td><td><b>'+data.fTUsage.toFixed(2)+'</b></td><td><b>'+data.fTDiesel.toFixed(2)+'</b></td><td><b>'+data.fTAmount.toFixed(2)+'</b></td><td><b>'+Number(fuelKmpl).toFixed(2)+'</b></td><td><b>'+costPerKM.toFixed(2)+'</b></td><td><b>'+diffAvgCost.toFixed(2)+'</b></td></tr></tbody></table></td></tr>';
		}else{
			incomeTable += '<tr><td class="key" colspan="5"><b>Total :</b></td><td><b>'+data.fTUsage.toFixed(2)+'</b></td><td><b>'+data.fTDiesel.toFixed(2)+'</b></td><td><b>'+data.fTAmount.toFixed(2)+'</b></td><td><b>'+Number(fuelKmpl).toFixed(2)+'</b></td><td><b>'+costPerKM.toFixed(2)+'</b></td></tr></tbody></table></td></tr>';
		}
		...
		if(!data.configuration.changedTripBalance){
			var  tripBalanceRow = '<tr><td><table class="table table-bordered table-striped"><thead></thead><tbody>';
			 tripBalanceRow += '<tr data-object-id="" class="ng-scope">'	
							   ...
							   +'<td colspan="0" class="value"><h4><i class="fa fa-inr"></i>'+(Number(TripSheet.tripTotalincome)+' -  <i class="fa fa-inr"></i>'+Number(data.tripTotalExpenseModified))+'  =   <i class="fa fa-inr"></i>'+(Number(TripSheet.tripTotalincome)-Number(data.tripTotalExpenseModified )).toFixed(2)+'</h4></td></tr></tbody></table></td></tr>';
		...
		if(data.configuration.showDriverBalance){
			...
			if(data.configuration.addInDriverbalanceAfterTripClose && TripSheet.tripStutesId > 2){
				if(TripSheet.closeTripStatusId == 1){
					tripBalanceRow3 += '<tr data-object-id="" class="ng-scope"><td><h4><b>Driver TripSheet Balance <span>(Advance - Expense)</span> </td><td>'+ (TripSheet.closeTripAmount+TripSheet.driverBalance).toFixed(2)  +' </b></h4></td></tr>';
