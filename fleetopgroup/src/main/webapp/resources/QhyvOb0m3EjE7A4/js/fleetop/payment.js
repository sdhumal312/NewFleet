...
let editPartyBankId = 0;
let editedPartyBankId = false;
...
function prepareSelectTwos(){
 $.getJSON("getAllBankNameist", {
         ...
         for (var b = '<option value="0"> </option>', c = a.length, d = 0; c > d; d++) b += '<option  id="_'+ a[d].bankId  + '"  value="' + a[d].bankId + '">' + a[d].bankName + "</option>";
...
function setBankPaymentDetails(button){
	if (allowBankPaymentDetails) {
		reset();
		if (transactionArray.includes(Number($(button).val()))) {
			getPaymentModalByPaymentType($(button).val());
			if(!buttonCreated){
			var test = $('<button/>', {
				id : 'bankPaymentDetailsButton',
				type: 'button',
			...
			reset();
			$('#bankPaymentTypeId').val($(button).val());
			buttonCreated = false;
			$('#bankPaymentDetailsButton').remove();
		}
	}
...
function setPaymentDetails(data,fromShow){
	...
	editPartyBankId=data.partyBankId;
...
function prepareObject(jsonObject){
	...
	if($('#partyBankId').val() != null && $('#partyBankId').val().trim() != '')
		object.partyBankId = $('#partyBankId').val();
	else
		object.partyBankId = 0;
...
function showPaymentModal() {
	...
	if( ($("#isEditpage").val() == "true" || $("#isEditpage").val()) && editedPartyBankId == false){	
		$("#partyBankId").val(editPartyBankId);
	    $("#" + editPartyBankId).prop("selected", true);
	    editedPartyBankId = true;
    }
