...
$(document).ready(function () {
	if(document.getElementById('salariedNo') != null){
		document.getElementById('salariedNo').checked = true;
	}
    $("#btnSubmit").on('keypress click', function (e) {
        e.preventDefault(); 	//stop submit the form, we will post it manually.
        validateAllField(addDriverFlag);
...
function showSelectedStatus(){
	$("input[name='driverStatusId']:checked").each(function(){
		driverStatusId = $(this).val();
	});
}
function saveDriveDetails(){
	showLayer();
	var array = new Array();
	array.push("Hindi");
	var jobType										= "";
	...
	if($('#driverLanguage').val() != undefined || $('#driverLanguage').val() != null) {	
	...
	/*renewal start*/
	jsonObject["driverRenewalTypeId"]			= $('#driverRenewalTypeId').val();
	jsonObject["driverReminderdate"]			= $('#DriverReminderdate').val();
	jsonObject["driverDueThreshold"]			= $('#driverDueThreshold').val();
	jsonObject["driverDueThresholdPeriod"]		= $('#driverDueThresholdPeriod').val();
	jsonObject["renewalDocId"]					= $('#renewalDocId').val();
	/*renewal end*/
    ...
    data.append("driverData", JSON.stringify(jsonObject)); 
    
	$.ajax({
		type: "POST",
		enctype: 'multipart/form-data',
		url: "saveDriverDetails",
		data: data,
		...
		success: function (data) {
			if(data.dlNumberExist != undefined && data.dlNumberExist == true){
				...
				hideLayer();
				return false;
			}else if(data.aadharValidate != undefined && data.aadharValidate == true){
				...
				setTimeout(function(){
					showMessage('success', 'New Driver Created Successfully!');	},3000);
				}
			},
		error: function (e) {
			hideLayer();
...
function autoEmpNumber(){
	...
	if($("#jobTypeWiseAutoGenerateEmpNo").val() == true || $("#jobTypeWiseAutoGenerateEmpNo").val() == 'true' || $("#autoGenerateEmpNo").val() == true || $("#autoGenerateEmpNo").val() == 'true'){
		...
		var jsonObject							= new Object();
		jsonObject["driJobId"]					= $("#driJobId").val().trim().split("-")[0]; 
		jsonObject["driJob"]					= $("#driJobId").val().trim().split("-")[1];
		$.ajax({
			url: "getCountByDriverJObId",
			type: "POST",
			dataType: 'json',
			data: jsonObject,
			success: function (data) {
