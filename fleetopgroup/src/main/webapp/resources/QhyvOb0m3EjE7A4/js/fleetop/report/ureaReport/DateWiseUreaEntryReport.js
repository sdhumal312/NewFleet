...
$(document).ready(
		function($) {
			$('button[type=submit]').click(function(e) {
				...
				$.ajax({					
					url: "getdateWiseUreaEntryReport.do",
					type: "POST",
					dataType: 'json',
					data: jsonObject,
...
function renderReportData(resultData) {
	...
	if(resultData.fuelEntryDetails != undefined){
		...
		$("#reportHeader").html("Fuel Entries Report");
...
function showUreaEntry(gridObj, dataView, row){

	var win = window.open('showUreaDetails.in?Id='+dataView.ureaEntriesId, '_blank');
	if (win) {
		win.focus();
	} else {
