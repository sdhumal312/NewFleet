            ...
            , results:function(a) {
                return {
                    results:$.map(a, function(a) {
                        return {
                            text: a.renewal_SubType, slug: a.slug, id: a.renewal_Subid
                        }
                    }
                    )
                }
...
$(document).ready(
		...
		function($) {
			$('button[type=submit]').click(function(e) {
				...
				jsonObject["renewalSubType"] 	  	=  $('#renewalSubType').val();
				jsonObject["renewalToDate"] 		=  $('#renewalToDate').val();
				
			
				$.ajax({
					
					url: "getRenewalExpiryDataReport",
					type: "POST",
					dataType: 'json',
					data: jsonObject,
...
function renderReportData(resultData) {
	...
	if(resultData.renewalList != undefined){
		...
		$('#reportHeader').html('<b>Renewal Sub Type Wise Expiry Report</b>');
