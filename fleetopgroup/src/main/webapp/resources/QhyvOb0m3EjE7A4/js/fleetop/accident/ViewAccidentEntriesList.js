$(document).ready(function() {
	getPageWiseVehicleAccidentDetails(1);
});

function getPageWiseVehicleAccidentDetails(pageNumber) {
	showLayer();
	var jsonObject					= new Object();
	jsonObject["pageNumber"]			= pageNumber;

	$.ajax({
		url: "getPageWiseVehicleAccidentDetails",
		type: "POST",
		dataType: 'json',
		data: jsonObject,
		success: function (data) {
			setAccidentInvoiceList(data);
			hideLayer();
		},
		error: function (e) {
			hideLayer();
...
function setAccidentInvoiceList(data) {
	...
	$('#countId').html('Total Vehicle Accident');
	var thead = $('<thead>');
	var tr1 = $('<tr>');
	...
	var tbody = $('<tbody>');
	if(data.accidentList != undefined && data.accidentList.length > 0) {
		
		var accidentList = data.accidentList;
		...
		for(var i = 0; i < accidentList.length; i++) {
			...
			tr1.append(td1.append('<a href="showVehicleAccidentDetails?id='+accidentList[i].encriptedId+'" target="_blank">AD-'+accidentList[i].accidentDetailsNumber+'</a>'));
			var curl = "showVehicle?vid="+accidentList[i].vid
			tr1.append(td2.append('<a target="_blank" href="' + curl + '">'+accidentList[i].vehicleNumber+'</a><br>'));
			tr1.append(td3.append(accidentList[i].driverName));
			tr1.append(td4.append(accidentList[i].accidentDateTimeStr));
			tr1.append(td5.append(accidentList[i].location));
			
			var curl1 = "editVehicleAccident.in?id="+accidentList[i].encriptedId
			...
			+'<li><a href="#" class="confirmation" onclick="deleteAccidentDetails('+accidentList[i].accidentDetailsId+')"><span class="fa fa-trash"></span> Delete</a></li>'
			+'</ul>'
			+'</div>'
			));
	...
	if(data.currentIndex == 1) {
		...
		$("#navigationBar").append('<li><a href="#" onclick="getPageWiseVehicleAccidentDetails(1)">&lt;&lt;</a></li>');		
	}

	if(data.currentIndex == 1) {
		$("#navigationBar").append('<li class="disabled"><a href="#" onclick="">&lt;</a></li>');
	} else {
		$("#navigationBar").append('<li><a href="#" onclick="getPageWiseVehicleAccidentDetails('+(data.currentIndex - 1)+')">&lt;</a></li>');
	}
	
	for (i = data.beginIndex; i <= data.endIndex; i++) {
	    if(i == data.currentIndex) {
	    	$("#navigationBar").append('<li class="active"><a href="#" onclick="getPageWiseVehicleAccidentDetails('+i+')">'+i+'</a></li>');	    	
	    } else {
	    	$("#navigationBar").append('<li><a href="#" onclick="getPageWiseVehicleAccidentDetails('+i+')">'+i+'</a></li>');	    	
	    }
	} 
	...
	if(data.deploymentLog.totalPages == 1) {
		...
		if(data.currentIndex == data.deploymentLog.totalPages) {
			...
			$("#navigationBar").append('<li><a href="#" onclick="getPageWiseVehicleAccidentDetails('+(data.deploymentLog.totalPages)+')">&gt;&gt;</a></li>');			
		}
	}
}
...
function deleteAccidentDetails(){
