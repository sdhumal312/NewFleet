...
function validateCloseAmount(){
	if($('#addInDriverbalanceAfterTripClose').val()  == 'true' || $('#addInDriverbalanceAfterTripClose').val() == true){
		var dBalance = Math.abs($('#driverTallyBalance').val()); 
		var cAmount = Number($('#closeTripAmount').val());
		if(cAmount > dBalance){
			//$('#closeTripAmount').val(0);
			 showMessage('info', 'Close Amount Cannot be grater than '+dBalance);
			 hideLayer();
			 $('#closeTripAmount').focus();
		  return false;
		}
	}
