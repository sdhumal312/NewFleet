...
function renderReportData(resultData) {
	if(resultData.PickAndDropList != undefined){
		...
		setSlickData(resultData.PickAndDropList, columnConfiguration, tableProperties);
...
function showTripSheetNumberDetails(gridObj, dataView, row){
	...
	var win;
	win = window.open('showDispatchedPickAndDropTrip.in?dispatchPickAndDropId='+dataView.tripsheetPickAndDropId, '_blank');
	if (win) {
	    win.focus();
	} else {
	    alert('Please allow popups for this website');
