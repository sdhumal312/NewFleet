			...
			$('button[type=submit]').click(function(e) {
				...
				jsonObject["vid"] 					= $('#vehicleId').val();
					...
					if($("#vehicleId").val() == "" || $("#vehicleId").val() == undefined ){
						hideLayer();
						showMessage('info','Please Enter Vehicle Rigistration');
						return false;
					}
				
				$.ajax({					
					url: "getMandatoryRenewalPendingReport",
					type: "POST",
					dataType: 'json',
					data: jsonObject,
...
function renderReportData(resultData) {
	...
	if(resultData.mandotoryRenewalPendingList != undefined){
		...
		$('#reportHeader').html('<b>Missing MandatoryRenewalReminder</b>');
		...
		setSlickData(resultData.mandotoryRenewalPendingList, columnConfiguration, tableProperties);
