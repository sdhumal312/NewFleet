	...
	<section class="content">
		...
		<div class="row">
			...
			<div class="col-md-9 col-sm-8 col-xs-12">
												...
												<c:choose>
													<c:when test="${mail.MAIL_IS_SEEN == false}">
														<tr class="success">
															...
															<td class="mailbox-name"><a
																href="<c:url value="/readmailbox?id=${mail.MAILBOX_ID_Encode}"/>"><small>To:
																		${mail.MAILBOX_TO_USER_NAME} <c:choose>
													...
													<c:otherwise>
														<tr class="active">
															...
															<td class="col-sm-5"><a
																href="<c:url value="/readmailbox?id=${mail.MAILBOX_ID_Encode}"/>"><small>
																		To: ${mail.MAILBOX_TO_USER_NAME} <c:choose>
																			<c:when test="${mail.MAIL_TEXTUAL_COUNT != 0}"> 
															(Re: ${mail.MAIL_TEXTUAL_COUNT} )
															</c:when>
																		</c:choose>
				...
				<!-- /. box -->
				<c:url var="firstUrl" value="/sentmailbox/1" />
				<c:url var="lastUrl"
					value="/sentmailbox/${deploymentLog.totalPages}" />
				<c:url var="prevUrl" value="/sentmailbox/${currentIndex - 1}" />
				<c:url var="nextUrl" value="/sentmailbox/${currentIndex + 1}" />
				<div class="text-center">
					<ul class="pagination pagination-lg pager">
						...
						<c:forEach var="i" begin="${beginIndex}" end="${endIndex}">
							<c:url var="pageUrl" value="/sentmailbox/${i}" />
							<c:choose>
