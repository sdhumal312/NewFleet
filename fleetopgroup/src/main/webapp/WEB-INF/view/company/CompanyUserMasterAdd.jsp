...
<div class="content-wrapper">
	...
	<section class="content">
		<c:if test="${param.already eq true}">
			<div class="alert alert-danger">
				...
				<h4>Warning!</h4>
				This User Name Already Created Successfully. Please Check.
			</div>
		</c:if>
									...
									<div class="form-horizontal ">
										<fieldset>
											...
											<div class="row1" id="grpfirstName" class="form-group">
												...
												<div class="I-size">
												<input name="CID" type="hidden"  value="${Company.company_id_encode}"
												Class="form-text" required="required" /> 
												<input name="COMPANY_CODE" type="hidden"  value="${Company.companyCode}"
												Class="form-text" required="required" /> 
											...
											<div class="form-group row1" id="grpuserEmail">
												...
												<div class="I-size">
													<input type="text" class="form-text " name="email"
														id="userId" maxlength="50" pattern="[a-zA-Z0-9]+"
														placeholder="eg: sachin"
														title="Invalid email please enter  user name"
														value="" required="required"
														onkeypress="" ondrop="return false;">
													...
													<span id="emailError" class="alert alert-danger col-sm-4"
														style="display: none">Already User Id
													...
													<select class="form-control select2" id="DepartmentList"
														...
														<c:forEach items="${department}" var="department">
																<option value="${department.depart_id}">
																		<c:out value="${department.depart_name}" />
																</option>
														</c:forEach>
													...
													<select class="form-control select2" id="BranchList"
														...
														<c:forEach items="${Branch}" var="Branch">
																<option value="${Branch.branch_id}">
																		<c:out value="${Branch.branch_name}" />
																</option>
														</c:forEach>
								...
								<fieldset class="form-actions">
									<div class="col-md-offset-1 col-md-5">
										...
										<a class="btn btn-info"
											href="<c:url value="/masterCompany/1.in"/>"><span
	...
	<script src="<c:url value="/resources/QhyvOb0m3EjE7A4/js/pwstrength.js" />"></script>
	...
	<script type="text/javascript">
		...
		function register(event) {
			...
			if ($("#password").val() != $("#matchPassword").val()) {
				...
				return;
			}else if (Number($('#DepartmentList').val()) == 0) {
				$("#userDepartmentErrorMsg")
				.show()
				.html('Please Select Department of User !');
				return;
		    }else if (Number($('#BranchList').val()) == 0) {
				$("#userBranchErrorMsg")
				.show()
				.html('Please Select Branch of User !');
			return;
		    }
			var formData = $('form').serialize();
			$
					.post(
							"<c:url value="/saveMasterUser"/>",
							formData,
							function(data) {
								if (data.message == "success") {
									window.location.href = '<c:url value="/masterCompany/1.html?success=true"></c:url>';
								} else if (data.message == "already") {
									window.location.href = '<c:url value="/masterCompany/1.html?already=true"></c:url>';
