		...
		<div class="row">
			...
			<sec:authorize access="hasAuthority('VIEW_CASHBOOK')">
				<div class="main-body">
											...
											<c:choose>
												<c:when test ="${Configuration.checkAuthorityForCashBook}" >
													...
													<sec:authorize access="hasAuthority('${authority}')">
													    <li role="presentation" id="${CashBookName.CASHBOOK_NAME}">
								...
								<table id="VendorTable" class="table table-hover table-bordered">
									<thead>
										<tr>
											...
											<th>Payment</th>
											<c:if test="${Configuration.showCashBookReference}">
												<th>Reference No.</th>
											</c:if>
											<th>Doc</th>
											<th>Actions</th>
										...
										<c:if test="${!empty CashBook}">
											<c:forEach items="${CashBook}" var="CashBook">
												...
												<c:if test="${Configuration.checkAuthorityForCashBook}">
													...
													<c:set var="newCashBookName" value="${fn:replace(CashBook.CASH_BOOK_NO,'-', '_')}" />
													<c:set var="authority" value="VIEW_${newCashBookName}" />
													<sec:authorize access="hasAuthority('${authority}')">
													...
													<tr data-object-id="" class="ng-scope showCashBookTableView" >
													<td class="fit"><a
					...
					<c:url var="firstUrl" value="/CashBookEntry/1" />
					<c:url var="lastUrl" value="/CashBookEntry/${deploymentLog.totalPages}" />
					<c:url var="prevUrl" value="/CashBookEntry/${currentIndex - 1}" />
					<c:url var="nextUrl" value="/CashBookEntry/${currentIndex + 1}" />
					<div class="text-center">
						<ul class="pagination pagination-lg pager">
							...
							<c:forEach var="i" begin="${beginIndex}" end="${endIndex}">
								<c:url var="pageUrl" value="/CashBookEntry/${i}" />
								<c:choose>
