...
class TripsheetVoucherDate extends StatefulWidget {

  final Function tripsheetShowData;

  TripsheetVoucherDate({this.tripsheetShowData, this.tripsheetId});
  final int tripsheetId;

  @override
  _TripsheetVoucherDateState createState() => _TripsheetVoucherDateState();
}

class _TripsheetVoucherDateState extends State<TripsheetVoucherDate> {
  ...
  TextEditingController voucherDate = new TextEditingController();
  ...
  Future addComment() async {
    if (!fieldValidation()) {
      ...
      var commentData = {
        ...
        'voucherDate': voucherDate.text,
      };

      var response = await ApiCall.getDataFromApi(
          URI.SAVE_TRIPSHEET_VOUCHER_DATE, commentData, URI.LIVE_URI, context);

      FlutterAlert.onSuccessAlert(context, " Tripsheet Voucher Date Added Successfully !", " Tripsheet Voucher Date ");
      refreshData();
  ...
  bool fieldValidation() {
    ...
    if (voucherDate.text == '' ) {
      FlutterAlert.onErrorAlert(
          context, "Please Select Voucher Date !", "Error");
      return false;
    }
