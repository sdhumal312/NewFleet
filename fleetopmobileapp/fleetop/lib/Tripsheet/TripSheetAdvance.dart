...
class TripsheetAdvance extends StatefulWidget {

  final Function tripsheetShowData;

  TripsheetAdvance({this.tripsheetShowData,this.tripsheetId});
  final int tripsheetId;

  @override
  _TripsheetAdvanceState createState() => _TripsheetAdvanceState();
}

class _TripsheetAdvanceState extends State<TripsheetAdvance> {
  ...
  Future addAdvance() async {
    if (!fieldValidation()) {
      ...
      var advanceData = {
        ...
        'driverAdvanceId': driverAdvanceId,
      ...
      var response = await ApiCall.getDataFromApi(
          URI.SAVE_ADVANCE_TRIPSHEET, advanceData, URI.LIVE_URI, context);

      if(response != null){
        if(response['updateAdvance'] != null && response['updateAdvance'] == true){
          FlutterAlert.onSuccessAlert(context, " Tripsheet Advance Added Successfully !", " Tripsheet Advance ");
        } else {
          FlutterAlert.onErrorAlert(context, "Advance Not Updated! Please contact Support", "Error");
        }
      }
      refreshData();
  ...
  bool fieldValidation() {
    ...
    if (advanceAmnt.text == '' || advanceAmnt.text == '0') {
//      FlutterAlert.onErrorAlert(
//          context, "Please Select Advance Amount Greater Than 0 !", "Error");
      Alert(
        context: context,
        type: AlertType.error,
        title: "Error",
        desc: "Please Select Advance Amount Greater Than 0 !",
        buttons: [
          DialogButton(
            child: Text(
              "OK",
...
//
  ...
  Future<bool> deleteAdvance(advanceData){
    Alert(
      context: context,
      type: AlertType.info,
      title: "Advance Info",
      desc: "Do you want to Delete Advance Details ?",
      buttons: [
        DialogButton(
          child: Text(
            "Yes",
          ...
          onPressed: () => deleteRequest(advanceData),
          gradient: LinearGradient(colors: [
            Colors.green,
            Colors.deepPurple
          ]),
  ...
  deleteRequest(advanceData) async{
    Navigator.pop(context);
  print("${advanceData}");
    var deleteAdvanceData = {
      'email': email,
      'userId': userId,
      'companyId': companyId,
      'tripsheetId': tripsheetId.toString(),
      'driverAdvanceId': driverAdvanceId,
      'tripAdvanceId' : '${advanceData['tripAdvanceID']}',
    };

    var response =  await ApiCall.getDataFromApi(
        URI.REMOVE_ADVANCE_TRIPSHEET, deleteAdvanceData, URI.LIVE_URI, context);
