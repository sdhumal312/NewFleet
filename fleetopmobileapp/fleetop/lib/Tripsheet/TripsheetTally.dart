...
class TripsheetTally extends StatefulWidget {

  final Function tripsheetShowData;

  TripsheetTally({this.tripsheetShowData, this.tripsheetId});
  final int tripsheetId;

  @override
  _TripsheetTallyState createState() => _TripsheetTallyState();
}

class _TripsheetTallyState extends State<TripsheetTally> {
  ...
  Future addComment() async {
    if (!fieldValidation()) {
      ...
      var response = await ApiCall.getDataFromApi(
          URI.SAVE_TRIPSHEET_TALLY_COMPANY, commentData, URI.LIVE_URI, context);

      FlutterAlert.onSuccessAlert(context, " Tripsheet Tally Company Added Successfully !", " Tripsheet Tally Company ");
      refreshData();
      ...
      if (response != null) {
        ...
        if (response["TallyCmpnyList"] != null) {
          ...
          for (int i = 0; i < response["TallyCmpnyList"].length; i++) {
            var obj = {
              "tallyID": response['TallyCmpnyList'][i]['tallyCompanyId'].toString(),
              "tallyName": response['TallyCmpnyList'][i]['companyName'].toString()
            };
