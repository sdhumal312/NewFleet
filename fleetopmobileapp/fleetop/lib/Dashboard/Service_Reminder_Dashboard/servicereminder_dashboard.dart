...
import 'package:fleetop/Dashboard/Service_Reminder_Dashboard/servicereminderlistview.dart';
...
class ServiceReminderDashboard extends StatefulWidget {
  ...
  @override
  _ServiceReminderDashboardDashboardState createState() => _ServiceReminderDashboardDashboardState();
}

class _ServiceReminderDashboardDashboardState extends State<ServiceReminderDashboard>
  ...
  String serviceCreatedCount;
  String todaysServiceCount;
  String selectedDate;
  bool showdate = false;
  Map serviceReminderData = Map();
  ...
  setInitialData() async {
    setState(() {
      startDate = DateTime.now();  // have put todays date and not taking date from earlier screen.
      ...
      serviceReminderData = widget.serviceReminderData;
    });
    getServiceReminderData(startDate, endDate);
    ...
    showDialog(
      ...
      builder: (BuildContext context) => CalendarPopupView(
        ...
        onApplyClick: (DateTime startData, DateTime endData) {
          ...
          getServiceReminderData(startData, endData);
        },
        onCancelClick: () {},
      ),
    );
  ...
  getServiceReminderData(startDate, endDate) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    companyId = prefs.getString("companyId");
    ...
    if (data != null) {
      setState(() {
        ...
        serviceCreatedCount =
            data["serviceReminderCreated"].toString();
        todaysServiceCount = 
            data["serviceReminderNotCreated"].toString();
         showdate = true;   
      });
      listViews.clear();
      addAllListData();
  ...
  setPieChartData() {
    bardata = [
      SummaryBar(
          summaryName: "Vehicles With SR",
          summaryCount: serviceCreatedCount,
          barColor: charts.ColorUtil.fromDartColor(Colors.blueAccent)),
      SummaryBar(
          summaryName: "Vehicles Without SR ",
          summaryCount: todaysServiceCount,
  ...
  addAllListData() {
    ...
    listViews.add(Container(
      ...
      child: ServiceReminderListView(
        ...
        serviceReminderData: serviceReminderData,
      ),
    ));
    ...
    listViews.add(Container(
      padding: EdgeInsets.only(top: 40),
      child: DaysWiseServiceReminderListView(
        startSelectedDate: startDate,
        endSelectedDate: endDate,
        mainScreenAnimation: widget.mainScreenAnimation,
        mainScreenAnimationController: widget.mainScreenAnimationController,
    ...
    listViews.add(Center(child: PieChartSummary(data: bardata,title : "Service Reminder Status")));
  }
