...
class ResolveIssue extends KFDrawerContent {
  final String issueId;
  bool navigateValue;
  ResolveIssue({Key key, this.issueId, this.navigateValue});

  @override
  _ResolveIssueState createState() => _ResolveIssueState();
}

class _ResolveIssueState extends State<ResolveIssue>
  ...
  Future _handleSubmitted() async {
    ...
    var data = await ApiCall.getDataFromApi(
        URI.RESOLVE_ISSUES_DETAILS, resolveDetails, URI.LIVE_URI, context);
    print("data =$data");
    if (data != null) {
      if (data['save'] == true) {
        refreshData();
        ...
        FlutterAlert.onInfoAlert(
            context, "Issue Not Resolved, Please contact on Support !", "Info");
      }
    } else {
  ...
  Future<bool> redirectToDisplay(issueId) async => Alert(
        ...
        title: "Success",
        desc: "Issue Successfully Resolved !",
        buttons: [
          DialogButton(
            child: Text(
              "OK",
