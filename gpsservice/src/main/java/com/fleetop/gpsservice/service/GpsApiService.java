...
public class GpsApiService {
	...
	public void getTatagpsDetails() {
		...
		try {
			...
			String url = String.format("https://demo.fleetop.in/restservices/vehicleListforTataGRP?gpsVendorId=%s&companyId=%s",
                    GpsApiConstant.TATAAPI, GpsApiConstant.COMPANY_CODE_STA);
			...
			if(response.getStatus() == 200) {
    			...
    			if(jsonObject.has("vehicleList") && !jsonObject.getJSONArray("vehicleList").isNull(0)) {
					...
					if (jSONArray != null) { 
						for (int i=0; i<jSONArray.length(); i++){ 
						    ...
						    vehicledto.setVid(object.getInt("vid"));
						    vehicledto.setVehicle_chasis(object.getString("vehicle_chasis"));
